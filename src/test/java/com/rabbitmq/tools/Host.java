...
public class Host {
	...
	private static final Pattern CONNECTION_NAME_PATTERN = Pattern
			.compile("\"connection_name\",\"(?<name>[a-zA-Z0-9\\-]+)?\"");
	...
	public static Process rabbitmqctlIgnoreErrors(String command) throws IOException {
		return executeCommandIgnoringErrors(rabbitmqctlCommand() + rabbitmqctlNodenameArgument() + " " + command);
	}
	...
	public static Process invokeMakeTarget(String command) throws IOException {
		...
		return executeCommand(
				makeCommand() + " -C \'" + rabbitmqDir() + "\'" + " RABBITMQCTL=\'" + rabbitmqctl.getAbsolutePath()
						+ "\'" + " RABBITMQ_NODENAME=\'" + nodenameA() + "\'" + " RABBITMQ_NODE_PORT=" + node_portA()
						+ " RABBITMQ_CONFIG_FILE=\'" + config_fileA() + "\'" + " " + command);
	}
	...
	public static class ConnectionInfo {
		...
		public String toString() {
			return "ConnectionInfo{" + "pid='" + pid + '\'' + ", peerPort=" + peerPort + ", clientProperties='"
					+ clientProperties + '\'' + ", clientProvidedName='" + clientProvidedName + '\'' + '}';
		}
	}
