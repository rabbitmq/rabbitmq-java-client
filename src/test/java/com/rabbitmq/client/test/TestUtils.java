...
public class TestUtils {
  ...
  public static boolean atMost312(Connection connection) {
    return atMostVersion(
        "3.12.999", currentVersion(connection.getServerProperties().get("version").toString()));
  }
  ...
  public static boolean basicGetBasicConsume(
      Connection connection, String queue, final CountDownLatch latch, int msgSize)
      throws Exception {
    Channel channel = connection.createChannel();
                ...
                .getOrComputeIfAbsent(
                    ...
                    k -> {
                      try (Connection c = TestUtils.connectionFactory().newConnection()) {
                        return currentVersion(c.getServerProperties().get("version").toString());
                      } catch (Exception e) {
                        throw new RuntimeException(e);
                      }
                    },
