...
public class AlternateExchange extends BrokerTestCase {
	...
	/**
	 * Determine which of the queues in our test configuration we expect a message
	 * with routing key <code>key</code> to get delivered to: the queue (if any)
	 * named <code>key</code>.
	 *
	 * @param key the routing key of the message
	 * @return an array of booleans that when zipped with {@link #resources}
	 *         indicates whether the messages is expected to be routed to the
	 *         respective queue
	 */
	...
	/**
	 * Declare an direct exchange <code>name</code> with an alternate-exchange
	 * <code>ae</code> and bind the queue <code>name</code> to it with a binding key
	 * of <code>name</code>.
	 *
	 * @param name the name of the exchange to be created, and queue to be bound
	...
	/**
	 * Perform an auto-acking 'basic.get' on each of the queues named in
	 * {@link #resources} and check whether a message can be retrieved when
	 * expected.
	 *
	 * @param expected an array of booleans that is zipped with {@link #resources}
	 *                 and indicates whether a messages is expected to be
	 *                 retrievable from the respective queue
	 */
	 ...
	 *
	 * We publish a message to exchange 'x' with a routing key of <code>key</code>,
	 * check whether the message (actually, any message) can be retrieved from the
	 * queues named in {@link #resources} when expected, and whether a
	 * 'basic.return' is received when expected.
	 *
	 * @param key       the routing key of the message to be sent
	 * @param mandatory whether the message should be marked as 'mandatory'
	 * @param expected  indicates which queues we expect the message to get routed
	 *                  to
	 * @param ret       whether a 'basic.return' is expected
	 *
	 * @see #checkGet(boolean[])
	 */
	protected void check(String key, boolean mandatory, boolean[] expected, boolean ret) throws IOException {

		gotReturn.set(false);
		channel.basicPublish("x", key, mandatory, false, null, "ae-test".getBytes());
		checkGet(expected);
		assertEquals(ret, gotReturn.get());
	}

	protected void check(String key, boolean[] expected, boolean ret) throws IOException {
		check(key, false, expected, ret);
	}

	protected void check(String key, boolean mandatory, boolean ret) throws IOException {
	...
	/**
	 * check various cases of missing AEs - we expect to see some warnings in the
	 * server logs
	 */
	@Test
