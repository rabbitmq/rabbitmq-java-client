...
/**
 * Test Requeue of messages on different types of close. Methods {@link #open}
 * and {@link #close} must be implemented by a concrete subclass.
 */
public abstract class RequeueOnClose extends BrokerTestCase {
	...
	/**
	 * Test close while consuming many messages successfully requeues unacknowledged
	 * messages
	 * 
	 * @throws Exception untested
	 */
	...
	/**
	 * Test close while consuming partially not acked with cancel successfully
	...
	/**
	 * Test close while consuming partially acked with cancel successfully requeues
	...
	/**
	 * Test close while consuming partially not acked without cancel successfully
	 * requeues unacknowledged messages
	 * 
	 * @throws Exception untested
	 */
	...
	/**
	 * Test close while consuming partially acked without cancel successfully
	 * requeues unacknowledged messages
	 * 
	 * @throws Exception untested
	 */
	...
	private void publishLotsAndConsumeSome(boolean ack, boolean cancelBeforeFinish)
		...
		for (int i = 0; i < requeuedMsgCount; i++) {
			assertNotNull("only got " + i + " out of " + requeuedMsgCount + " messages", channel.basicGet(Q, true));
		}
		int countMoreMsgs = 0;
		while (null != channel.basicGet(Q, true)) {
			countMoreMsgs++;
		}
		assertTrue("got " + countMoreMsgs + " more messages than " + requeuedMsgCount + " expected",
				0 == countMoreMsgs);
		channel.queueDelete(Q);
		close();
		closeConnection();
