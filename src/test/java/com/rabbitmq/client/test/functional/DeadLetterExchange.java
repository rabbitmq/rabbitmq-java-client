...
public class DeadLetterExchange extends BrokerTestCase {
	...
	@Test
	public void declareQueueWithExistingDeadLetterExchange() throws IOException {
		declareQueue(DLX);
	}

	@Test
	public void declareQueueWithNonExistingDeadLetterExchange() throws IOException {
		declareQueue("some.random.exchange.name");
	}

	@Test
	public void declareQueueWithEquivalentDeadLetterExchange() throws IOException {
		declareQueue(DLX);
		declareQueue(DLX);
	}

	@Test
	public void declareQueueWithEquivalentDeadLetterRoutingKey() throws IOException {
	...
	@Test
	public void redeclareQueueWithInvalidDeadLetterExchangeArg() throws IOException {
	...
	@Test
	public void redeclareQueueWithInvalidDeadLetterRoutingKeyArg() throws IOException {
	...
	@Test
	public void deadLetterMultipleDeadLetterQueues() throws IOException {
		declareQueue(TEST_QUEUE_NAME, DLX, null, null, 1);
	...
	public void deadLetterNewRK() throws Exception {
		...
		publishN(MSG_COUNT, (new AMQP.BasicProperties.Builder()).headers(headers).build());
		...
		consumeN(DLQ2, MSG_COUNT, getResponse -> {
			...
			assertDeathReason(death, 0, TEST_QUEUE_NAME, "expired", "amq.direct", Arrays.asList("test", "foo"));
		});
	}
	...
	public void republish() throws Exception {
		...
		assertNotNull(death);
		assertEquals(1, death.size());
		assertDeathReason(death, 0, TEST_QUEUE_NAME, "expired", "amq.direct", Collections.singletonList("test"));
		...
		channel.basicPublish("amq.direct", "test", new AMQP.BasicProperties.Builder().headers(headers).build(),
		...
		assertDeathReason(death, 0, TEST_QUEUE_NAME, "maxlen", "amq.direct", Collections.singletonList("test"));
		assertDeathReason(death, 1, TEST_QUEUE_NAME, "expired", "amq.direct", Collections.singletonList("test"));
	...
	private void rejectionTest(final boolean useNack) throws Exception {
		deadLetterTest((Callable<Void>) () -> {
			for (int x = 0; x < MSG_COUNT; x++) {
				GetResponse getResponse = channel.basicGet(TEST_QUEUE_NAME, false);
	...
	private void deadLetterTest(final Runnable deathTrigger, Map<String, Object> queueDeclareArgs, String reason)
			throws Exception {
		deadLetterTest(() -> {
			deathTrigger.run();
			return null;
	...
	private void deadLetterTest(Callable<?> deathTrigger, Map<String, Object> queueDeclareArgs, final String reason)
			throws Exception {
	...
	public static void consume(final Channel channel, final String reason) throws IOException {
		consumeN(channel, DLQ, MSG_COUNT, getResponse -> {
			...
			assertEquals(1, death.size());
			assertDeathReason(death, 0, TEST_QUEUE_NAME, reason, "amq.direct", Collections.singletonList("test"));
		});
	}
	...
	/*
	 * check that each message arrives within epsilon of the publication time + TTL
	 * + latency
	 */
	private void checkPromptArrival(AccumulatingMessageConsumer c, int count, long latency) throws Exception {
		long epsilon = TTL / 5;
		for (int i = 0; i < count; i++) {
			...
			assertTrue("expiry outside bounds (+/- " + epsilon + "): " + (now - targetTime),
					(now >= targetTime - epsilon) && (now <= targetTime + epsilon));
		}
	}
	...
	private void declareQueue(String queue, Object deadLetterExchange, Object deadLetterRoutingKey,
			Map<String, Object> args) throws IOException {
		declareQueue(queue, deadLetterExchange, deadLetterRoutingKey, args, 0);
	}

	private void declareQueue(String queue, Object deadLetterExchange, Object deadLetterRoutingKey,
			Map<String, Object> args, int ttl) throws IOException {
		if (args == null) {
			args = new HashMap<>();
		}
	...
	private void publishN(int n, AMQP.BasicProperties props) throws IOException {
		for (int x = 0; x < n; x++) {
			publish(props, "test message");
		}
	}

	private void publish(AMQP.BasicProperties props, String body) throws IOException {
	...
	private void consumeN(String queue, int n, WithResponse withResponse) throws IOException {
		consumeN(channel, queue, n, withResponse);
	}

	private static void consumeN(Channel channel, String queue, int n, WithResponse withResponse) throws IOException {
		for (int x = 0; x < n; x++) {
			GetResponse getResponse = channel.basicGet(queue, true);
			assertNotNull("Messages not dead-lettered (" + (n - x) + " left)", getResponse);
			assertEquals("test message", new String(getResponse.getBody()));
	...
	@SuppressWarnings("unchecked")
	private static void assertDeathReason(List<Object> death, int num, String queue, String reason, String exchange,
			List<String> routingKeys) {
		Map<String, Object> deathHeader = (Map<String, Object>) death.get(num);
	...
	@SuppressWarnings("unchecked")
	private static void assertDeathReason(List<Object> death, int num, String queue, String reason) {
		Map<String, Object> deathHeader = (Map<String, Object>) death.get(num);
