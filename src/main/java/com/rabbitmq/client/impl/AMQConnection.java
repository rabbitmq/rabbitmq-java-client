...
public class AMQConnection extends ShutdownNotifierComponent implements Connection, NetworkConnection {
	...
	// the heartbeat one, so we use a value of 105% of the effective heartbeat
	// timeout
	...
	private final List<RecoveryCanBeginListener> recoveryCanBeginListeners = Collections
	...
	/**
	 * Retrieve a copy of the default table of client properties that will be sent
	 * to the server during connection startup. This method is called when each new
	 * ConnectionFactory instance is constructed.
	 * 
	...
	/**
	 * Flag indicating whether the client received Connection.Close message from the
	 * broker
	 */
	...
	public AMQConnection(ConnectionParams params, FrameHandler frameHandler, MetricsCollector metricsCollector) {
		...
		this.trafficListener = params.getTrafficListener() == null ? TrafficListener.NO_OP
				: params.getTrafficListener();
	...
	private void initializeConsumerWorkService() {
		this._workService = new ConsumerWorkService(consumerWorkServiceExecutor, threadFactory, workPoolTimeout,
				shutdownTimeout);
	}
	...
	/**
	 * Start up the connection, including the MainLoop thread. Sends the protocol
	 * version negotiation header, and runs through Connection.Start/.StartOk,
	 * Connection.Tune/.TuneOk, and then calls Connection.Open and waits for the
	 * OpenOk. Sets heart-beat and frame max values after tuning has taken place.
	 * 
	 * @throws IOException if an error is encountered either before, or during,
	 *                     protocol negotiation; sub-classes
	 *                     {@link ProtocolVersionMismatchException} and
	 *                     {@link PossibleAuthenticationFailureException} will be
	 *                     thrown in the corresponding circumstances.
	 *                     {@link AuthenticationFailureException} will be thrown if
	 *                     the broker closes the connection with ACCESS_REFUSED. If
	 *                     an exception is thrown, connection resources allocated
	 *                     can all be garbage collected when the connection object
	...
	public void start() throws IOException, TimeoutException {
			...
			if (credentialsProvider.getTimeBeforeExpiration() != null) {
				if (this.credentialsRefreshService == null) {
					throw new IllegalStateException(
							"Credentials can expire, a credentials refresh service should be set");
				}
				if (this.credentialsRefreshService
						.isApproachingExpiration(credentialsProvider.getTimeBeforeExpiration())) {
		...
		try {
			...
			if (!checkUnsignedShort(negotiatedChannelMax)) {
				throw new IllegalArgumentException("Negotiated channel max must be between 0 and " + MAX_UNSIGNED_SHORT
			...
			if (!checkUnsignedShort(negotiatedHeartbeat)) {
				throw new IllegalArgumentException("Negotiated heartbeat must be between 0 and " + MAX_UNSIGNED_SHORT
						+ ": " + negotiatedHeartbeat);
			}

			setHeartbeat(negotiatedHeartbeat);

			_channel0.transmit(new AMQP.Connection.TuneOk.Builder().channelMax(negotiatedChannelMax).frameMax(frameMax)
					.heartbeat(negotiatedHeartbeat).build());
		...
		if (this.credentialsProvider.getTimeBeforeExpiration() != null) {
			String registrationId = this.credentialsRefreshService.register(credentialsProvider, () -> {
				// return false if connection is closed, so refresh service can get rid of this
				// registration
				if (!isOpen()) {
					return false;
				}
			...
			addShutdownListener(
					sse -> this.credentialsRefreshService.unregister(this.credentialsProvider, registrationId));
		}
	...
	protected void configureChannelManager(ChannelManager channelManager) {
		...
		channelManager
				.setChannelShutdownTimeout((int) ((requestedHeartbeat * CHANNEL_SHUTDOWN_TIMEOUT_MULTIPLIER) * 1000));
	}
	 ...
	 * Public API
	 *
	 * @return true if this work service instance uses its own
	 *         consumerWorkServiceExecutor (as opposed to a shared one)
	 */
	...
	private static int negotiatedMaxValue(int clientValue, int serverValue) {
		return (clientValue == 0 || serverValue == 0) ? Math.max(clientValue, serverValue)
	...
	private class MainLoop implements Runnable {
		...
		/**
		 * Channel reader thread main loop. Reads a frame, and if it is not a heartbeat
		 * frame, dispatches it to the channel it refers to. Continues running until the
		 * "running" flag is set false by shutdown().
		 */
		@Override
		public void run() {
	...
	public void handleHeartbeatFailure() {
		Exception ex = new MissedHeartbeatException("Heartbeat missing with heartbeat = " + _heartbeat + " seconds");
		try {
	...
	/**
	 * Protected API - causes all attached channels to terminate (shutdown) with a
	 * ShutdownSignal built from the argument, and stops this connection from
	 ...
	 * @param notifyRpc              true if outstanding rpc should be informed of
	 *                               shutdown
	 * @return a shutdown signal built using the given arguments
	 */
	public ShutdownSignalException shutdown(Method reason, boolean initiatedByApplication, Throwable cause,
			boolean notifyRpc) {
	 ...
	 * and timeout value for all the close operations to complete. Specifies if any
	 * encountered exceptions should be ignored.
	 */
	public void close(int closeCode, String closeMessage, boolean initiatedByApplication, Throwable cause, int timeout,
			boolean abort) throws IOException {
		boolean sync = !(Thread.currentThread() == mainLoopThread);

		try {
			AMQP.Connection.Close reason = new AMQP.Connection.Close.Builder().replyCode(closeCode)
					.replyText(closeMessage).build();
	...
	public String toString() {
		...
		return "amqp://" + this.credentialsProvider.getUsername() + "@" + getHostAddress() + ":" + getPort()
				+ virtualHost;
	}
