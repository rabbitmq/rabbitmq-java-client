 ...
 * not close channels on unhandled consumer and listener exception.
 * 
...
public class ForgivingExceptionHandler implements ExceptionHandler {
	...
	public void handleConsumerException(Channel channel, Throwable exception, Consumer consumer, String consumerTag,
			String methodName) {
		handleChannelKiller(channel, exception, "Consumer " + consumer + " (" + consumerTag + ")" + " method "
				+ methodName + " for channel " + channel);
	}
	...
	protected void log(String message, Throwable e) {
		if (isSocketClosedOrConnectionReset(e)) {
			...
			LoggerFactory.getLogger(ForgivingExceptionHandler.class)
					.warn(message + " (Exception message: " + e.getMessage() + ")");
		} else {
