...
public abstract class AMQChannel extends ShutdownNotifierComponent {
	 ...
	 * hierarchy). In the meantime, this at least won't throw away any information
	...
	CompletableFuture<Command> exnWrappingAsyncRpc(Method m) throws IOException {
		try {
		...
		if (!processAsync(command)) {
			...
			if (_checkRpcResponseType) {
				synchronized (_channelMutex) {
					// check if this reply command is intended for the current waiting request
					// before calling nextOutstandingRpc()
					if (_activeRpc != null && !_activeRpc.canHandleReply(command)) {
						// this reply command is not intended for the current waiting request
						// most likely a previous request timed out and this command is the reply for
						// that.
						// Throw this reply command away so we don't stop the current request from
	...
	/**
	 * Protected API - sends a {@link Method} to the broker and waits for the next
	 * in-bound Command from the broker: only for use from non-connection-MainLoop
	 * threads!
	 */
	...
	/**
	 * Cleans RPC channel state after a timeout and wraps the TimeoutException in a
	...
	void quiescingRpc(Method m, RpcContinuation k) throws IOException {
		synchronized (_channelMutex) {
			enqueueRpc(k);
			quiescingTransmit(m);
		}
	...
	private void asyncRpc(Method m, CompletableFuture<Command> future) throws IOException {
		synchronized (_channelMutex) {
			ensureIsOpen();
	...
	private void quiescingAsyncRpc(Method m, CompletableFuture<Command> future) throws IOException {
		synchronized (_channelMutex) {
	 ...
	 * Command before it is returned to the caller of nextCommand. If this method
	 * returns true, the command is considered handled and is not passed back to
	 * nextCommand's caller; if it returns false, nextCommand returns the command as
	 * usual. This is used in subclasses to implement handling of Basic.Return and
	 * Basic.Deliver messages, as well as Channel.Close and Connection.Close.
	 * 
	 * @param command the command to handle asynchronously
	 * @return true if we handled the command; otherwise the caller should consider
	 *         it "unhandled"
	 */
	 ...
	 * @param ignoreClosed the flag indicating whether to ignore the
	 *                     AlreadyClosedException thrown when the channel is already
	 *                     closed
	 * @param notifyRpc    the flag indicating whether any remaining rpc
	 *                     continuation should be notified with the given signal
	 */
	...
	public static abstract class BlockingRpcContinuation<T> implements RpcContinuation {
		...
		static boolean isResponseCompatibleWithRequest(Method request, Method response) {
			// make a best effort attempt to ensure the reply was intended for this rpc
			// request
			// Ideally each rpc request would tag an id on it that could be returned and
			// referenced on its reply.
			// But because that would be a very large undertaking to add passively this
			// logic at least protects against ClassCastExceptions
			if (request != null) {
				if (request instanceof Basic.Qos) {
					...
					if (!(response instanceof Basic.ConsumeOk))
						return false;
					// can also check the consumer tags match here. handle case where request
					// consumer tag is empty and server-generated.
					final String consumerTag = ((Basic.Consume) request).getConsumerTag();
					return consumerTag == null || consumerTag.equals("")
							|| consumerTag.equals(((Basic.ConsumeOk) response).getConsumerTag());
				} else if (request instanceof Basic.Cancel) {
					if (!(response instanceof Basic.CancelOk))
						return false;
					// can also check the consumer tags match here
					return ((Basic.Cancel) request).getConsumerTag()
							.equals(((Basic.CancelOk) response).getConsumerTag());
					...
					// see QueueLifecycle test and
					// https://github.com/rabbitmq/rabbitmq-server/issues/710
