...
/**
 * Represents an AMQP wire-protocol frame, with frame type, channel number, and
 * payload bytes.
 */
public class Frame {
	/** Frame type code */
	...
	private static final int NON_BODY_SIZE = 1 /* type */ + 2 /* channel */ + 4 /* payload size */
			+ 1 /* end character */;

	/**
	 * Constructs a frame for output with a type and a channel number and a fresh
	 * accumulator waiting for payload.
	 */
	...
	/**
	 * Constructs a frame for input with a type, a channel number and a payload byte
	 * array.
	 */
	...
	public static Frame readFrom(DataInputStream is) throws IOException {
		...
		if (type == 'A') {
			 ...
			 */
			/*
			 * Otherwise meaningless, so try to read the version, and throw an exception,
			 * whether we read the version okay or not.
			 */
	...
	/**
	 * Private API - A protocol version mismatch is detected by checking the three
	 * next bytes if a frame type of (int)'A' is read from an input stream. If the
	 * next three bytes are 'M', 'Q' and 'P', then it's likely the broker is trying
	 * to tell us we are speaking the wrong AMQP protocol version.
	 *
	 * @throws MalformedFrameException if an AMQP protocol version mismatch is
	 *                                 detected
	...
	public static void protocolVersionMismatch(DataInputStream is) throws IOException {
		...
		// We expect the letters M, Q, P in that order: generate an informative error if
		// they're not found
		...
		try {
			...
			if (signature[0] == 1 && signature[1] == 1 && signature[2] == 8 && signature[3] == 0) {
				x = new MalformedFrameException("AMQP protocol version mismatch; we are version " + AMQP.PROTOCOL.MAJOR
						+ "-" + AMQP.PROTOCOL.MINOR + "-" + AMQP.PROTOCOL.REVISION + ", server is 0-8");
			} else {
				String sig = "";
				for (int i = 0; i < 4; i++) {
					if (i != 0)
				...
				x = new MalformedFrameException("AMQP protocol version mismatch; we are version " + AMQP.PROTOCOL.MAJOR
						+ "-" + AMQP.PROTOCOL.MINOR + "-" + AMQP.PROTOCOL.REVISION + ", server sent signature " + sig);
			}
	...
	/**
	 * Public API - retrieves a fresh DataOutputStream streaming into the
	 * accumulator
	 */
	...
	private static long fieldValueSize(Object value) throws UnsupportedEncodingException {
		...
		if (value instanceof String) {
			...
			throw new IllegalArgumentException("invaSlid value in table");
		}
		return acc;
	}

	/**
	 * Computes the AMQP 0-9-1 wire-protocol length of an encoded field-array of
	 * type List
	 */
	...
	/**
	 * Computes the AMQP wire-protocol length of an encoded field-array of type
	 * Object[]
	 */
