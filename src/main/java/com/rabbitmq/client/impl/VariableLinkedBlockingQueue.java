...
/**
 * A clone of {@linkplain java.util.concurrent.LinkedBlockingQueue} with the
 * addition of a {@link #setCapacity(int)} method, allowing us to change the
 * capacity of the queue while it is in use.
 * <p>
 *
 ...
 *
 * <p>
 * This class implements all of the <em>optional</em> methods of the
...
public class VariableLinkedBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable {
	...
	/**
	 * Signal a waiting take. Called only from put/offer (which do not otherwise
	 ...
	 * @throws IllegalArgumentException if <tt>capacity</tt> is not greater than
	 *                                  zero.
	 */
	...
	/**
	 * Set a new capacity for the queue. Increasing the capacity can cause any
	 * waiting {@link #put(Object)} invocations to succeed if the new capacity is
	 ...
	 * to the initial capacity of this queue less the current <tt>size</tt> of this
	 * queue.
	 * <p>
	 * Note that you <em>cannot</em> always tell if an attempt to <tt>add</tt> an
	 * element will succeed by inspecting <tt>remainingCapacity</tt> because it may
	 * be the case that a waiting consumer is ready to <tt>take</tt> an element out
	 * of an otherwise full queue.
	 */
	@Override
	 ...
	 *
	 * @param o the element to add.
	 * @return <tt>true</tt> if it was possible to add the element to this queue,
	 *         else <tt>false</tt>
	 * @throws NullPointerException if the specified element is <tt>null</tt>
	 */
	@Override
	 ...
	 * returned <tt>Iterator</tt> is a "weakly consistent" iterator that will never
	 * throw {@link java.util.ConcurrentModificationException}, and guarantees to
	...
	private class Itr implements Iterator<E> {
		/*
		 * Basic weak-consistent iterator. At all times hold the next item to hand out
	 ...
	 *             (each an <tt>Object</tt>) in the proper order, followed by a null
	 * @param s the stream
	 */
