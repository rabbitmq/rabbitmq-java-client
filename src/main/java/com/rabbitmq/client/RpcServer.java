...
public class RpcServer {
	 ...
	 * Connection) is shut down, or until terminateMainloop() is called, or until
	 * the thread running the loop is interrupted.
	 *
	 * Note that if the mainloop is blocked waiting for a request, the termination
	 * flag is not checked until a request is received, so a good time to call
	...
	/**
	 * High-level response method. Returns an empty response by default - override
	 * this (or other handleCall and handleCast methods) in subclasses.
	 */
	...
	/**
	 * Gives a chance to set/modify reply properties before handling call. Note the
	 * correlationId property is already set.
	 * 
	...
	/**
	 * High-level handler method. Does nothing by default - override this (or other
	 * handleCast and handleCast methods) in subclasses.
	 */
