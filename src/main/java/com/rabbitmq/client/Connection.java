...
/**
 * Public API: Interface to an AMQ connection. See the see the
 * <a href="https://www.amqp.org/">spec</a> for details.
 * <p>
 * To connect to a broker, fill in a {@link ConnectionFactory} and use a
 * {@link ConnectionFactory} as follows:
 *
 * <pre>
 ...
 * </pre>
 *
 * Current implementations are thread-safe for code at the client API level, and
 * in fact thread-safe internally except for code within RPC calls.
 */
public interface Connection extends ShutdownNotifier, Closeable { // rename to AMQPConnection later, this is a temporary
																	// name
	/**
	 * Retrieve the host.
	 * 
	...
	/**
	 * Get the negotiated maximum channel number. Usable channel numbers range from
	...
	/**
	 * Returns client-provided connection name, if any. Note that the value returned
	 * does not uniquely identify a connection and cannot be used as a connection
	 * identifier in HTTP API requests.
	 *
	 *
	 *
	...
	/**
	 * Close this connection and all its channels with the
	 * {@link com.rabbitmq.client.AMQP#REPLY_SUCCESS} close code and message 'OK'.
	 *
	 ...
	 * {@link com.rabbitmq.client.AMQP#REPLY_SUCCESS} close code and message 'OK'.
	 * 
	 * This method behaves in a similar way as {@link #close()}, with the only
	 * difference that it waits with a provided timeout for all the close operations
	 * to complete. When timeout is reached the socket is forced to close.
	 * 
	 ...
	 * Close this connection and all its channels.
	 *
	 * Waits with the given timeout for all the close operations to complete. When
	 * timeout is reached the socket is forced to close.
	 * 
	 * @param closeCode    the close code (See under "Reply Codes" in the AMQP
	 *                     specification)
	 * @param closeMessage a message indicating the reason for closing the
	 *                     connection
	 * @param timeout      timeout (in milliseconds) for completing all the
	 *                     close-related operations, use -1 for infinity
	 * @throws IOException if an I/O problem is encountered
	 */
	void close(int closeCode, String closeMessage, int timeout) throws IOException;

	/**
	 * Abort this connection and all its channels with the
	 * {@link com.rabbitmq.client.AMQP#REPLY_SUCCESS} close code and message 'OK'.
	 *
	 * Forces the connection to close. Any encountered exceptions in the close
	 * operations are silently discarded.
	 */
	void abort();
	 ...
	 *
	 * Forces the connection to close and waits for all the close operations to
	 * complete. Any encountered exceptions in the close operations are silently
	 * discarded.
	 * 
	 * @param closeCode    the close code (See under "Reply Codes" in the AMQP
	 *                     specification)
	 ...
	 * {@link com.rabbitmq.client.AMQP#REPLY_SUCCESS} close code and message 'OK'.
	 *
	 * This method behaves in a similar way as {@link #abort()}, with the only
	 * difference that it waits with a provided timeout for all the close operations
	 * to complete. When timeout is reached the socket is forced to close.
	 *
	 * @param timeout timeout (in milliseconds) for completing all the close-related
	 ...
	 * Abort this connection and all its channels.
	 *
	 * Forces the connection to close and waits with the given timeout for all the
	 * close operations to complete. When timeout is reached the socket is forced to
	 * close. Any encountered exceptions in the close operations are silently
	 * discarded.
	 *
	 * @param closeCode    the close code (See under "Reply Codes" in the AMQP
	 *                     specification)
	 ...
	 *
	 * This ID must be unique, otherwise some services like the metrics collector
	 * won't work properly. This ID doesn't have to be provided by the client,
