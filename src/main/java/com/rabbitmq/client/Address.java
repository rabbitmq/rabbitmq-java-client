...
/**
 * A representation of network addresses, i.e. host/port pairs, with some
 * utility functions for parsing address strings.
 */
public class Address {
	...
	/**
	 * Extracts hostname or IP address from a string containing a hostname, IP
	 * address, hostname:port pair or IP address:port pair. Note that IPv6 addresses
	 * must be quoted with square brackets, e.g.
	...
	public static String parseHost(String addressString) {
		...
		if (lastClosingSquareBracket == -1) {
			...
			if (parts.length > 2) {
				String msg = "Address " + addressString
	...
	/**
	 * Factory method: takes a formatted addressString string as construction
	 * parameter
	 * 
	...
	/**
	 * Array-based factory method: takes an array of formatted address strings as
