...
/**
 * Convenience factory class to facilitate opening a {@link Connection} to a
 * RabbitMQ node.
 *
 * Most connection and socket settings are configured using this factory. Some
 * settings that apply to connections can also be configured here and will apply
 * to all connections produced by this factory.
 */
public class ConnectionFactory implements Cloneable {
	...
	/**
	 * Default maximum channel number; 2047 because it's 2048 on the server side
	 * minus channel 0, which each connection uses for negotiation and error
	 * communication
	 */
	...
	/**
	 * The default AMQP 0-9-1 connection handshake timeout. See
	 * DEFAULT_CONNECTION_TIMEOUT for TCP (socket) connection timeout.
	 */
	...
	/**
	 * Whether or not channels check the reply type of an RPC call. Default is
	 * false.
	 * 
	 * @since 4.2.0
	 */
	...
	/**
	 * Listener called when a connection gets an IO error trying to write on the
	 * socket. Default listener triggers connection recovery asynchronously and
	...
	/**
	 * Set a custom credentials provider. Default implementation uses static
	 * username and password.
	 * 
	 * @param credentialsProvider The custom implementation of CredentialsProvider
	 ...
	 * username, password and virtual host. If any part of the URI is omitted, the
	 ...
	 * Convenience method for setting the fields in an AMQP URI: host, port,
	 * username, password and virtual host. If any part of the URI is omitted, the
	 * ConnectionFactory's corresponding variable is left unchanged. Note that not
	 * all valid AMQP URIs are accepted; in particular, the hostname must be given
	 * if the port, username or password are given, and escapes in the hostname are
	 * not permitted.
	 * 
	...
	/**
	 * Convenience method for setting some fields from query parameters Will handle
	 * only a subset of the query parameters supported by the official erlang client
	 * https://www.rabbitmq.com/uri-query-parameters.html
	 * 
	 * @param rawQuery is the string containing the raw query parameters part from a
	 *                 URI
	 */
	...
	/**
	 * Hook to process query parameters not handled natively. Handled natively:
	 * <code>heartbeat</code>, <code>connection_timeout</code>,
	 ...
	 *
	 * @param requestedChannelMax initially requested maximum channel number; zero
	 *                            for unlimited
	 */
	 ...
	 * 
	 * @param requestedFrameMax initially requested maximum frame size, in octets;
	 *                          zero for unlimited
	 */
	 ...
	 * 
	 * @param timeout connection TCP establishment timeout in milliseconds; zero for
	 *                infinite
	 */
	...
	/**
	 * Set the shutdown timeout. This is the amount of time that Consumer
	 * implementations have to continue working through deliveries (and other
	 * Consumer callbacks) <b>after</b> the connection has closed but before the
	 * ConsumerWorkService is torn down. If consumers exceed this timeout then any
	 * remaining queued deliveries (and other Consumer callbacks, <b>including</b>
	 * the Consumer's handleShutdownSignal() invocation) will be lost.
	 * 
	 * @param shutdownTimeout shutdown timeout in milliseconds; zero for infinite;
	 *                        default 10000
	 */
	...
	/**
	 * Set the requested heartbeat timeout. Heartbeat frames will be sent at about
	 * 1/2 the timeout interval. If server heartbeat timeout is configured to a
	 * non-zero value, this method can only be used to lower the value; otherwise
	 * any value provided by the client will be used.
	 * <p>
	 * Note the value must be between 0 and 65535 (unsigned short in AMQP 0-9-1).
	 *
	 * @param requestedHeartbeat the initially requested heartbeat timeout, in
	 *                           seconds; zero for none
	 * @see <a href="https://rabbitmq.com/heartbeats.html">RabbitMQ Heartbeats
	 *      Guide</a>
	 */
	...
	/**
	 * Retrieve the currently-configured table of client properties that will be
	 * sent to the server during connection startup. Clients may add, delete, and
	 * alter keys in this table. Such changes will take effect when the next new
	...
	/**
	 * Replace the table of client properties that will be sent to the server during
	 * subsequent connection startups.
	 * 
	 ...
	 * javax.net.ssl.SSLSocketFactory instance. Note this applies only to blocking
	 * IO, not to NIO, as the NIO API doesn't use the SocketFactory API.
	 * 
	 * @see #useSslProtocol
	 */
	 ...
	 * before it has been opened. The default socket configurator disables Nagle's
	 * algorithm.
	 *
	...
	/**
	 * Set the executor to use for consumer operation dispatch by default for newly
	 * created connections. All connections that use this executor share it.
	 *
	 * It's developer's responsibility to shut down the executor when it is no
	 * longer needed.
	 *
	...
	/**
	 * Set the executor to use for connection shutdown. All connections that use
	 * this executor share it.
	 *
	 * It's developer's responsibility to shut down the executor when it is no
	 * longer needed.
	 *
	...
	/**
	 * Set the executor to use to send heartbeat frames. All connections that use
	 * this executor share it.
	 *
	 * It's developer's responsibility to shut down the executor when it is no
	 * longer needed.
	...
	/**
	 * Convenience method for configuring TLS using the default set of TLS protocols
	 * and a trusting TrustManager. This setup is <strong>only suitable for
	 * development and QA environments</strong>. The trust manager will
	 * <strong>trust every server certificate presented</strong> to it, this is
	 * convenient for local development but <strong>not recommended to use in
	 * production</strong> as it provides no protection against man-in-the-middle
	 * attacks. Prefer {@link #useSslProtocol(SSLContext)}.
	 */
	...
	/**
	 * Convenience method for configuring TLS using the supplied protocol and a very
	 * trusting TrustManager. This setup is <strong>only suitable for development
	 * and QA environments</strong>. The trust manager <strong>will trust every
	 * server certificate presented</strong> to it, this is convenient for local
	 * development but not recommended to use in production as it <strong>provides
	 * no protection against man-in-the-middle attacks</strong>.
	 *
	 * Use {@link #useSslProtocol(SSLContext)} in production environments. The
	 * produced {@link SSLContext} instance will be shared by all the connections
	...
	/**
	 * Convenience method for configuring TLS. Pass in the TLS protocol version to
	 * use, e.g. "TLSv1.2" or "TLSv1.1", and a desired {@link TrustManager}.
	 *
	 *
	 * The produced {@link SSLContext} instance will be shared with all the
	 * connections created by this connection factory. Use
	 ...
	 * for setting up the context with a {@link TrustManager} with suitable security
	 * guarantees, e.g. peer verification.
	 *
	 *
	 * The {@link SSLContext} instance will be shared with all the connections
	 * created by this connection factory. Use
	 * {@link #setSslContextFactory(SslContextFactory)} for more flexibility.
	 * 
	 ...
	 * <p>
	 * This enables hostname verification regardless of the IO mode used (blocking
	 * or non-blocking IO).
	 * <p>
	 * This can be called typically after setting the {@link SSLContext} with one of
	 * the <code>useSslProtocol</code> methods.
	 *
	 ...
	 * connection recovery</a> is enabled, false otherwise
	 * 
	 * @return true if automatic connection recovery is enabled, false otherwise
	 * @see <a href="https://www.rabbitmq.com/api-guide.html#recovery">Automatic
	 *      Recovery</a>
	 */
	...
	/**
	 * Get the executor to use for parallel topology recovery. If null (the
	 * default), recovery is done single threaded on the main connection thread.
	 * 
	...
	/**
	 * Set the executor to use for parallel topology recovery. If null (the
	 * default), recovery is done single threaded on the main connection thread. It
	 * is recommended to pass a ThreadPoolExecutor that will allow its core threads
	 * to timeout so these threads can die when recovery is complete. It's
	 * developer's responsibility to shut down the executor when it is no longer
	 * needed. Note: your
	 * {@link ExceptionHandler#handleTopologyRecoveryException(Connection, Channel, TopologyRecoveryException)}
	...
	/**
	 * Set a {@link CredentialsRefreshService} instance to handle credentials
	 * refresh if appropriate.
	 * <p>
	 * Each created connection will register to the refresh service to send an AMQP
	 * <code>update.secret</code> frame when credentials are about to expire. This
	 * is the refresh service responsibility to schedule credentials refresh and
	 * <code>udpate.secret</code> frame sending, based on the information provided
	 * by the {@link CredentialsProvider}.
	 * <p>
	 * Note the {@link CredentialsRefreshService} is used only when the
	 * {@link CredentialsProvider} signals credentials can expire, by returning a
	 * non-null value from {@link CredentialsProvider#getTimeBeforeExpiration()}.
	 *
	...
	protected synchronized FrameHandlerFactory createFrameHandlerFactory() throws IOException {
		if (nio) {
			if (this.frameHandlerFactory == null) {
				...
				this.frameHandlerFactory = new SocketChannelFrameHandlerFactory(connectionTimeout, nioParams, isSSL(),
						sslContextFactory);
			}
	...
	/**
	 * Create a new broker connection, picking the first available address from the
	 * list.
	 *
	 * If <a href="https://www.rabbitmq.com/api-guide.html#recovery">automatic
	 * connection recovery</a> is enabled, the connection returned by this method
	 * will be {@link Recoverable}. Future reconnection attempts will pick a random
	 * accessible address from the provided list.
	 *
	 * @param addrs an array of known broker addresses (hostname/port pairs) to try
	 *              in order
	...
	/**
	 * Create a new broker connection, picking the first available address from the
	 * list provided by the {@link AddressResolver}.
	 *
	 * If <a href="https://www.rabbitmq.com/api-guide.html#recovery">automatic
	 * connection recovery</a> is enabled, the connection returned by this method
	 * will be {@link Recoverable}. Future reconnection attempts will pick a random
	 * accessible address provided by the {@link AddressResolver}.
	 *
	 * @param addressResolver discovery service to list potential addresses
	 *                        (hostname/port pairs) to connect to
	 * @return an interface to the connection
	 * @throws IOException if it encounters a problem
	 * @see <a href="https://www.rabbitmq.com/api-guide.html#recovery">Automatic
	 *      Recovery</a>
	...
	/**
	 * Create a new broker connection with a client-provided name, picking the first
	 * available address from the list.
	 *
	 * If <a href="https://www.rabbitmq.com/api-guide.html#recovery">automatic
	 * connection recovery</a> is enabled, the connection returned by this method
	 * will be {@link Recoverable}. Future reconnection attempts will pick a random
	 * accessible address from the provided list.
	 *
	 * @param addrs              an array of known broker addresses (hostname/port
	 *                           pairs) to try in order
	 * @param clientProvidedName application-specific connection name, will be
	 *                           displayed in the management UI if RabbitMQ server
	 *                           supports it. This value doesn't have to be unique
	 *                           and cannot be used as a connection identifier e.g.
	 *                           in HTTP API requests. This value is supposed to be
	 *                           human-readable.
	 * @return an interface to the connection
	 * @throws IOException if it encounters a problem
	 */
	 ...
	 * accessible address from the provided list.
	 *
	 * @param addrs a List of known broker addresses (hostname/port pairs) to try in
	 *              order
	 * @return an interface to the connection
	 * @throws IOException if it encounters a problem
	 */
	 ...
	 * accessible address from the provided list.
	 *
	 * @param addrs              a List of known broker addresses (hostname/port
	 *                           pairs) to try in order
	 * @param clientProvidedName application-specific connection name, will be
	 *                           displayed in the management UI if RabbitMQ server
	 *                           supports it. This value doesn't have to be unique
	 ...
	 * @param addrs    an array of known broker addresses (hostname/port pairs) to
	 *                 try in order
	 * @return an interface to the connection
	 ...
	 * accessible address from the provided list.
	 *
	 * @param executor           thread execution service for consumers on the
	 *                           connection
	 * @param addrs              an array of known broker addresses (hostname/port
	 *                           pairs) to try in order
	 * @param clientProvidedName application-specific connection name, will be
	 ...
	 *
	 * @param executor thread execution service for consumers on the connection
	 * @param addrs    a List of known broker addrs (hostname/port pairs) to try in
	 *                 order
	 * @return an interface to the connection
	 * @throws java.io.IOException if it encounters a problem
	 * @see <a href="https://www.rabbitmq.com/api-guide.html#recovery">Automatic
	 ...
	 * @param executor           thread execution service for consumers on the
	 *                           connection
	 * @param addrs              a List of known broker addrs (hostname/port pairs)
	 *                           to try in order
	 * @param clientProvidedName application-specific connection name, will be
	 *                           displayed in the management UI if RabbitMQ server
	 *                           supports it. This value doesn't have to be unique
	...
	/**
	 * Create a new broker connection with a client-provided name, picking the first
	 * available address from the list provided by the {@link AddressResolver}.
	 *
	 * If <a href="https://www.rabbitmq.com/api-guide.html#recovery">automatic
	 * connection recovery</a> is enabled, the connection returned by this method
	 * will be {@link Recoverable}. Future reconnection attempts will pick a random
	 ...
	 * If <a href="https://www.rabbitmq.com/api-guide.html#recovery">automatic
	 * connection recovery</a> is enabled, the connection returned by this method
	 * will be {@link Recoverable}. Reconnection attempts will always use the
	 * address configured on {@link ConnectionFactory}.
	 *
	 * @return an interface to the connection
	 * @throws IOException if it encounters a problem
	 */
	 ...
	 * address configured on {@link ConnectionFactory}.
	 *
	 * @param connectionName client-provided connection name (an arbitrary string).
	 *                       Will be displayed in management UI if the server
	 *                       supports it.
	 * @return an interface to the connection
	 * @throws IOException if it encounters a problem
	 */
	...
	/**
	 * Load settings from a property file. Keys must be prefixed with
	 * <code>rabbitmq.</code>, use {@link ConnectionFactory#load(String, String)} to
	...
	/**
	 * Load settings from a {@link Properties} instance. Keys must be prefixed with
	 * <code>rabbitmq.</code>, use
	 * {@link ConnectionFactory#load(Properties, String)} to specify your own
	 * prefix.
	 * 
	...
	/**
	 * Load settings from a {@link Map} instance. Keys must be prefixed with
	 * <code>rabbitmq.</code>, use {@link ConnectionFactory#load(Map, String)} to
	 * specify your own prefix.
	 * 
	 * @param properties source for settings
	 * @since 4.4.0
	 ...
	 * 
	 * @return how long will automatic recovery wait before attempting to reconnect,
	...
	/**
	 * Sets connection recovery interval. Default is 5000. Uses
	 * {@link com.rabbitmq.client.RecoveryDelayHandler.DefaultRecoveryDelayHandler}
	 * by default. Use another {@link RecoveryDelayHandler} implementation for more
	 * flexibility.
	 * 
	 * @param networkRecoveryInterval how long will automatic recovery wait before
	...
	/**
	 * Use non-blocking IO (NIO) for communication with the server. With NIO,
	 * several connections created from the same {@link ConnectionFactory} can use
	 * the same IO thread.
	 *
	 * A client process using a lot of not-so-active connections can benefit from
	 * NIO, as it would use fewer threads than with the traditional, blocking IO
	 * mode.
	 *
	 * Use {@link NioParams} to tune NIO and a {@link SocketChannelConfigurator} to
	 * configure the underlying {@link java.nio.channels.SocketChannel}s for
	 * connections.
	 *
	 * @see NioParams
	...
	/**
	 * Use blocking IO for communication with the server. With blocking IO, each
	 * connection creates its own thread to read data from the server.
	 */
	...
	/**
	 * Set the continuation timeout for RPC calls in channels. Default is 10
	...
	/**
	 * The factory to create SSL contexts. This provides more flexibility to create
	 * {@link SSLContext}s for different connections than sharing the
	 * {@link SSLContext} with all the connections produced by the connection
	 * factory (which is the case with the {@link #useSslProtocol()} methods). This
	 * way, different connections with a different certificate for each of them is a
	 * possible scenario.
	 * 
	 ...
	 * expects a queue.declare-ok response) of RPC calls and ignore those that do
	...
	/**
	 * Timeout (in ms) for work pool enqueueing. The
	 * {@link com.rabbitmq.client.impl.WorkPool} dispatches several types of
	 * responses from the broker (e.g. deliveries). A high-traffic client with slow
	 * consumers can exhaust the work pool and compromise the whole connection (by
	 * e.g. letting the broker saturate the receive TCP buffers). Setting a timeout
	 * would make the connection fail early and avoid hard-to-diagnose TCP
	 * connection failure. Note this shouldn't happen with clients that set
	 * appropriate QoS values. Default is no timeout.
	 *
	...
	/**
	 * Set a listener to be called when connection gets an IO error trying to write
	 * on the socket. Default listener triggers connection recovery asynchronously
	 * and propagates the exception. Override the default listener to disable or
	...
	/**
	 * Allows to decide on automatic connection recovery is triggered. Default is
	 * for shutdown not initiated by application or missed heartbeat errors.
	 *
	...
	/**
	 * Set the recovered queue name supplier. Default is use the same queue name
	 * when recovering queues.
	 * 
