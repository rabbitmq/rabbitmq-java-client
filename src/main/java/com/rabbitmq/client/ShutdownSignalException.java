 ...
 * either get a reference to the Connection or Channel instance that fired this
 * exception.
 */
...
public class ShutdownSignalException extends RuntimeException implements SensibleClone<ShutdownSignalException> {
	...
	/**
	 * True if the connection is shut down, or false if this signal refers to a
	 * channel
	 */
	private final boolean _hardError;

	/**
	 * True if this exception is caused by explicit application action; false if it
	 * originated with the broker or as a result of detectable non-deliberate
	 * application failure
	 */
	 ...
	 * @param ref                    Reference to Connection or Channel that fired
	 *                               the signal
	 */
	...
	private static String composeMessage(boolean hardError, boolean initiatedByApplication, Method reason,
			String messagePrefix, Throwable cause) {
	...
	/**
	 * @return true if this exception was caused by explicit application action;
