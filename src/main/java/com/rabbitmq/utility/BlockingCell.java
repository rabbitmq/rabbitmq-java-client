...
/**
 * Simple one-shot IPC mechanism. Essentially a one-place buffer that cannot be
 * emptied once filled.
 */
public class BlockingCell<T> {
	...
	/**
	 * Wait for a value, and when one arrives, return it (without clearing it). If
	 * there's already a value present, there's no need to wait - the existing value
	 * is returned.
	 * 
	 ...
	 * Wait for a value, and when one arrives, return it (without clearing it). If
	 * there's already a value present, there's no need to wait - the existing value
	 * is returned. If timeout is reached and value hasn't arrived, TimeoutException
	 * is thrown.
	 * 
	...
	/**
	 * As get(), but catches and ignores InterruptedException, retrying until a
	 * value appears.
	 * 
	 * @return the waited-for value
	 */
	...
	/**
	 * As get(long timeout), but catches and ignores InterruptedException, retrying
	 * until a value appears or until specified timeout is reached. If timeout is
	 * reached, TimeoutException is thrown. We also use System.nanoTime() to behave
	 * correctly when system clock jumps around.
	 * 
	...
	/**
	 * Store a value in this BlockingCell, throwing {@link IllegalStateException} if
	 ...
	 * 
	 * @return true if this call to setIfUnset actually updated the BlockingCell;
	 *         false if the cell already had a value.
	 * @param newValue the new value to store
	 */
