...
/**
 * <p>
 * A class for allocating integers from a given range that uses a {@link BitSet}
 * representation of the free integers.
 * </p>
 *
 * <h2>Concurrency Semantics:</h2> This class is <b><i>not</i></b> thread safe.
 *
 * <h2>Implementation notes:</h2>
 * <p>
 * This was originally an ordered chain of non-overlapping Intervals, together
 * with a fixed size array cache for freed integers.
 * </p>
 * <p>
 * {@link #reserve(int)} was expensive in this scheme, whereas in the present
 * implementation it is O(1), as is {@link #free(int)}.
 * </p>
 * <p>
 * Although {@link #allocate()} is slightly slower than O(1) and in the worst
 * case could be O(N), the use of a "<code>lastIndex</code>" field for starting
 * the next scan for free integers means this is negligible.
 * </p>
 * <p>
...
public class IntAllocator {
	...
	/**
	 * A bit is SET in freeSet if the corresponding integer is FREE <br/>
	...
	/**
	 * Allocate an unallocated integer from the range, or return -1 if no more
	...
	/**
	 * Make the provided integer available for allocation again. This operation runs
	 * in O(1) time. No error checking is performed, so if you double free or free
	 * an integer that was not originally allocated the results are undefined.
	 * 
	 ...
	 * if it is available, false otherwise. This operation runs in O(1) time.
	 * 
	 * @param reservation the integer to be allocated, if possible
	 * @return <code><b>true</b></code> if allocated, <code><b>false</b></code> if
	 *         already allocated
	 */
	...
	public String toString() {
		...
		if (firstClearBit < this.numberOfBits) {
			...
			for (int i = this.freeSet.nextClearBit(firstSetAfterThat + 1); i < this.numberOfBits; i = this.freeSet
					.nextClearBit(i + 1)) {
